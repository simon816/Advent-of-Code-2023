...
def get_adj(pos, dir, in_count):
    ...
    if in_count < 3:
        only_dir = dir
    if (r - 1, c) in nodes and dir != 'S' and (only_dir is None or only_dir == 'N'):
        count = in_count + 1 if dir == 'N' else 0
        if count < 10:
            yield (r - 1, c), 'N', count
    if (r, c - 1) in nodes and dir != 'E' and (only_dir is None or only_dir == 'W'):
        count = in_count + 1 if dir == 'W' else 0
        if count < 10:
            yield (r, c - 1), 'W', count
    if (r + 1, c) in nodes and dir != 'N' and (only_dir is None or only_dir == 'S'):
        count = in_count + 1 if dir == 'S' else 0
        if count < 10:
            yield (r + 1, c), 'S', count
    if (r, c + 1) in nodes and dir != 'W' and (only_dir is None or only_dir == 'E'):
